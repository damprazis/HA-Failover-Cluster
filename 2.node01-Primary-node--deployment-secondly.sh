# This is a script to create 2-node Active\Passive HA cluster with KVM guest on it and and manage replication with DRBD.
#!/bin/bash
#
#
if [ "$(whoami)" != "root" ]
...
lvcreate -L 200G -n drbd0 cl -y

lvcreate -L 200G -n drbd1 cl -y

lvcreate -L 100G -n drbd2 cl -y

lvcreate -L 100G -n drbd3 cl -y



dd if=/dev/zero of=/dev/mapper/cl-drbd0 bs=1024k count=1024

dd if=/dev/zero of=/dev/mapper/cl-drbd1 bs=1024k count=1024

dd if=/dev/zero of=/dev/mapper/cl-drbd2 bs=1024k count=1024

dd if=/dev/zero of=/dev/mapper/cl-drbd3 bs=1024k count=1024
...
common {
        handlers {
                ...
                # pri-on-incon-degr "/usr/lib/drbd/notify-pri-on-incon-degr.sh; /usr/l$
                # pri-lost-after-sb "/usr/lib/drbd/notify-pri-lost-after-sb.sh; /usr/l$
                # local-io-error "/usr/lib/drbd/notify-io-error.sh; /usr/lib/drbd/noti$
...
resource r3 {
    device    /dev/drbd3;
    disk      /dev/mapper/cl-drbd3;
    meta-disk internal;
  on nordnode01.localdomain {
    address   10.0.10.253:7799;
  }
  on nordnode02.localdomain {
...
drbdadm primary --force r00

drbdadm primary --force r30
...
cat /dev/null > /etc/libvirt/qemu.conf
...
rsync -av /etc/libvirt/qemu.conf root@node02:/etc/libvirt/qemu.conf
...
echo ""
echo "######################################################################################"
echo "Create a file system on the DRBD resource"
sudo mkfs.xfs /dev/drbd0 && sudo mkfs.xfs /dev/drbd1 && sudo mkfs.xfs /dev/drbd2 && sudo mkfs.xfs /dev/drbd3

echo ""
echo "######################################################################################"
echo "Part 6. Authenticating all nodes for the cluster"

pcs host auth node01 node02

pcs cluster setup HAcluster node01 node02
